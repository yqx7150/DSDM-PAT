...
from utilis.diffusion_utils.utils import restore_checkpoint, fft2, ifft2, show_samples_gray, get_mask, clear
...
def config_parser():
    ...
    parser.add_argument("--config1", default=f"config/Diffusion_config/AAPM_256_DR_256x2_1000.yaml")
    parser.add_argument("--config2", default=f"config/Nerf_config/Lineformer/AAPM_256_D256X2.yaml")
    return parser
...
def iteration(config,image_pred,image,geo,angles,save_root):
    ...
    # 创建各种类型的保存目录
    irl_types = ['input', 'recon', 'label', 'kspace', 'sinogram',"img"]
    for t in irl_types:
        if t == 'recon':
    ...
    kspace = fft2(img)
    # generate mask
    mask = get_mask(torch.zeros(1, 1, h, w), h, b,
                    type=mask_type, acc_factor=acc_factor, center_fraction=center_fraction)
    mask = mask.to(img.device)
    mask = mask.repeat(b, 1, 1, 1)
    pc_fouriercs = controllable_generation_TV.get_pc_radon_ADMM_TV_temp(sde,
                                                                        predictor, corrector,
                                                                        inverse_scaler,
                                                                        mask=mask,
                                                                        lamb_1=lamb,
    ...
    for i, recon_img in enumerate(x):
        plt.imsave(os.path.join(save_root, 'kspace', f'{count}.png'), clear(kspace[i]), cmap='gray')
