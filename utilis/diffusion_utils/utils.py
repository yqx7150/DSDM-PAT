...
import numpy as np
from function.diffusion_function.fastmri_utils import fft2c_new, ifft2c_new
from statistics import mean, stdev
from skimage.metrics import peak_signal_noise_ratio, structural_similarity
from sporco.metric import gmsd, mse
from scipy.ndimage import gaussian_laplace
import functools
import utilis.diffusion_utils.sde_lib as sde_lib  # 导入随机微分方程（SDE）库
...
def initSDE(config):
  if config['diffusion_training']['sde'].lower() == 'vpsde':
      sde = sde_lib.VPSDE(beta_min=config['diffusion_model']['beta_min'], beta_max=config['diffusion_model']['beta_max'],
                          N=config['diffusion_model']['num_scales'])  # 创建VPSDE对象
  elif config['diffusion_training']['sde'].lower() == 'subvpsde':
      sde = sde_lib.subVPSDE(beta_min=config['diffusion_model']['beta_min'], beta_max=config['diffusion_model']['beta_max'],
                              N=config['diffusion_model']['num_scales'])  # 创建subVPSDE对象
  elif config['diffusion_training']['sde'].lower() == 'vesde':
      sde = sde_lib.VESDE(sigma_min=config['diffusion_model']['sigma_min'], sigma_max=config['diffusion_model']['sigma_max'],
                          N=config['diffusion_model']['num_scales'])  # 创建VESDE对象
  else:
      raise NotImplementedError(f"SDE {config['diffusion_training']['sde']} unknown.")  # 如果SDE类型未知，抛出错误
